...
function buildUrlForEndpoint(
  ...
  const pathOptions =
    endpoint.path === "/configs" ? configPathSegments : (endpoint.pathOptions ?? []);
  ...
  for (const param of endpoint.queryParams) {
    ...
    if (path.includes("/models/{param}") && name === "id") continue;
    if (name === "id" && hasIdRange) continue;
...
export default function ApiDocsPage() {
    ...
    async function load() {
      ...
      try {
        const [endpointsData] = await Promise.all([
          fetchApiJson<EndpointsData>("/endpoints/data"),
          fetchApiJson<{ pathSegments?: string[] }>("/config-types")
            .then((v) => {
              if (!cancelled && Array.isArray(v.pathSegments)) {
                setConfigPathSegments(v.pathSegments);
              }
            })
            .catch(() => {
              // Optional endpoint; keep fallback.
            }),
          fetchApiJson<{ revisionOptions?: string[]; serverRevision?: number }>("/revisions")
            .then((v) => {
              ...
              if (typeof v.serverRevision === "number") {
                setServerRevision(v.serverRevision);
              }
            })
            .catch(() => {
              // Optional endpoint.
        ...
        const categories = Array.from(
          new Set(endpointsData.endpoints.map((endpoint) => endpoint.category)),
        ).sort();
        if (categories.length > 0) {
          setActiveCategory(categories[0]);
        }
        const initialOpen = new Set<string>();
        for (const endpoint of endpointsData.endpoints.slice(0, 3)) {
          initialOpen.add(createEndpointKey(endpoint));
        }
  ...
  const setParam = React.useCallback(
    (endpointKey: string, name: string, value: string) => {
      setParamValues((prev) => ({
        ...prev,
  ...
  React.useEffect(() => {
    if (!testModalOpen && testResponse && typeof testResponse === "object" && testResponse) {
        ...
        <Card>
          <CardContent className="space-y-4 pt-4">
            {(grouped[activeCategory] ?? []).map((endpoint) => {
              ...
              const builtUrl = buildUrlForEndpoint(
                endpoint,
                data.baseUrl,
                values,
                configPathSegments,
                ...
                >
                  <div className="rounded-lg border bg-muted/20">
                    ...
                    >
                      ...
                      <Badge className={cn("font-mono", methodColor(endpoint.method))}>
                      ...
                      <div className="space-y-4 border-t px-3 py-3">
                        ...
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                          {endpoint.queryParams.map((param) => {
                            if (endpoint.path.includes("/models/{param}") && param.name === "id") {
                              return null;
                            }
                            const inputId = `${endpointKey}-${param.name}`;
                            const value = values[param.name] ?? "";
                            const isBool = param.type.toLowerCase().includes("boolean");
                            const isRev = param.name === "rev";
                            const isIdRange = param.name === "idRange";
                                ...
                                {isBool ? (
                                  ...
                                  >
                                    {revisionOptions.map((option) => (
                                      <option key={option} value={option}>
                                        {option === "latest" && serverRevision != null
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        setParam(endpointKey, ID_RANGE_PARTS[0], e.target.value)
                                      }
                                      className="h-8 px-2 text-xs"
                                    />
                                    ...
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        setParam(endpointKey, ID_RANGE_PARTS[1], e.target.value)
                                      }
                                      className="h-8 px-2 text-xs"
                                    />
                                  </div>
                        ...
                        <div className="rounded-md border border-border bg-zinc-950/60 p-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              variant="ghost"
                              size="icon-sm"
                              onClick={() => runQuery(endpoint.method, builtUrl)}
                              aria-label="Run endpoint query"
                            >
        ...
        <DialogContent className="max-h-[85vh] max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription className="space-y-1">
              <span className={cn("inline-flex rounded border px-2 py-0.5 text-xs font-medium", methodColor(testMethod))}>
                {testMethod}
              </span>
          ...
          <div className="min-h-0 flex-1 overflow-auto rounded border bg-muted/25 p-3">
            {testLoading ? (
              ...
              <img
                src={(testResponse as { url: string }).url}
                alt="Endpoint response"
