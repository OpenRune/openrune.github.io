...
async function handleProxy(
  ...
  try {
    ...
    try {
      ...
      // Get response body as arrayBuffer to handle both text and binary (images) correctly
      ...
      // Handle connection errors gracefully (e.g., ECONNREFUSED, timeouts)
      ...
      if (errorCode === 'ECONNREFUSED' || errorCode === 'ETIMEDOUT' || errorName === 'AbortError') {
        return NextResponse.json(
          { error: 'Service unavailable', message: 'Cache server is not responding' },
          { status: 503 }
        );
      }
